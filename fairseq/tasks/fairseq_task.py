...
class FairseqTask(object):
    ...
    def get_dropout_batch(self, sample, model, criterion, optimizer, tok_func, task): 
        ...
        sample_dropped['net_input']['src_tokens'], sample_dropped['net_input']['src_lengths'] = self.get_dropout(sample, tok_func, task.source_dictionary, for_src=True)
        sample_dropped['net_input']['prev_output_tokens'], sample_dropped['target'] = self.get_dropout(sample, tok_func, task.target_dictionary, for_src=False)       
        
        return sample_dropped
    

    def get_dropout(self, batch, tok_func, dictionary, for_src):
        
        if for_src:
            batch_idx = batch['net_input']['src_tokens'].cpu().numpy()
            batch_size = batch['net_input']['src_tokens'].size(0)
        else:
            batch_idx = batch['net_input']['prev_output_tokens'].cpu().numpy()
            batch_size = batch['net_input']['src_tokens'].size(0)
        ...
        max_sequence = 0
        for b in batch_idx:
            sent = dictionary.string(b[(b != dictionary.pad()) & (b != dictionary.eos()) & (b != dictionary.bos()) & (b != dictionary.unk())])
        ...
        if not for_src:
            droped_tgt_idx         = np.array(droped_tgt_idx)
            droped_prev_tgt_padded = list(map(lambda x: self.pad_seq_tgt(x, max_sequence, dictionary), droped_batch_idx))            
            droped_tgt_padded      = list(map(lambda x: self.pad_seq_tgt(x, max_sequence+1, dictionary), droped_tgt_idx))
            droped_prev_tgt_padded = torch.tensor(np.array(droped_prev_tgt_padded)).long().to(device)
            droped_tgt_padded      = torch.tensor(np.array(droped_tgt_padded)).long().to(device)
            droped_prev_tgt_tokens = torch.cat([eos, droped_prev_tgt_padded], dim = 1)

            return droped_prev_tgt_tokens, droped_tgt_padded

        else:
            droped_src_padded = list(map(lambda x: self.pad_seq_src(x, max_sequence, dictionary), droped_batch_idx))
            droped_src_padded = torch.tensor(np.array(droped_src_padded)).long().to(device)
            droped_src_tokens = torch.cat([droped_src_padded, eos], dim = 1)
            droped_src_length = torch.sum(droped_src_tokens!=dictionary.pad(), dim =1)

            return droped_src_tokens, droped_src_length       
