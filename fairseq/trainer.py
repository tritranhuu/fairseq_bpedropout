...
class Trainer(object):
    ...
    def train_step_dropout(self, samples, tok_func, raise_oom=False):
        self._set_seed()
        self.model.train()
        self.criterion.train()
        self.zero_grad()

        metrics.log_start_time("train_dropout_wall", priority=800, round=0)
        ...
        for i, dropout_sample in enumerate(dropout_samples):  # delayed update loop
            sample, is_dummy_batch = self._prepare_sample(dropout_sample)
            try:
                with maybe_no_sync():
